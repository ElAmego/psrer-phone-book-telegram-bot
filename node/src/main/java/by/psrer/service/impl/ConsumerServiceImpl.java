...
public final class ConsumerServiceImpl implements ConsumerService {
    ...
    @Override
    @RabbitListener(queues = SENT_MESSAGE)
    public void consumeSentMessage(final Message sentMessage) {
        log.debug("NODE: sentMessage is received");
        sentMessageService.handleSentMessage(sentMessage);
    }
}